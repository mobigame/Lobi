...
void CreateUnityRenderBuffers(UnityRenderingSurface* surface)
{
    ...
    [[KLMVideoCapture sharedInstance] setCurrentContext:surface->context withGLView:UnityGetGLView()];
    [[KLMVideoCapture sharedInstance] createFramebuffer:surface->systemFB];
    
    [KLMVideoCapture sharedInstance].activeFramebufferCallback = ^(GLuint activeFramebuffer){
...
{
    BOOL isCapturing = [[KLMVideoCapture sharedInstance] isCapturing];
    ...
    // 録画フレームを追加する
        ...
        [[KLMVideoCapture sharedInstance] appendFrame:surface->systemFB];
    }
